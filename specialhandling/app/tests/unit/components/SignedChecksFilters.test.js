...
describe('SignedChecksFilters.test.js', () => {
    let wrapper;
        ...
        rowData: [
            {
                ...
                picked_up: false,
                signature: {
                    created: "2018-06-04T16:11:45.747282",
                    first_name: "Students",
                    id: 1,
                    last_name: "Everywhere",
                    modified: "2018-06-04T16:11:45.747531",
                    related_check: 2,
                    signature: "dlkfjsdlkfjsdlkfjdsl",
                    user: "r" ,
                },
                user: "r"
            }
    ...
    beforeEach(() => {
        wrapper = mount(SignedChecksFilters, {
            localVue,
            propsData: {
                ...props
            },
    ...
    it('filters by `date_received`', () => {
        const input = wrapper.find('#selectDateReceived');
        const value = '2018-06-04';
        input.element.value = value;
        input.trigger('input');
        expect(wrapper.vm.filters.date_received.filter).toBe(value);
        const expectedFilterModel = {
            date_received: {
                type: 'contains',
                filter: value,
    ...
    it('filters `picked_up`', () => {
        const input = wrapper.findAll(SegmentedToggle).at(1);
        input.findAll(SegmentedToggleOption).at(0).trigger('click');
        const value = 'true';
        expect(wrapper.vm.filters.picked_up.value).toBe(value);
        const expectedFilterModel = {
            picked_up: {
                value: value
            }
    ...
    it('filters by `received_by`', () => {
        const input = wrapper.find('#selectReceivedBy');
        const value = 'Students Everywhere';
        input.element.value = value;
        input.trigger('input');
        expect(wrapper.vm.filters.received_by.filter).toBe(value);
        const expectedFilterModel = {
            received_by: {
                type: 'contains',
                filter: value,
